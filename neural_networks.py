...
class ConvNetStream(torch.nn.Module):
    def __init__(self, optical_flow_stream = False, out_features = 512) -> None:
        ...
        self.conv4 = torch.nn.Conv2d(in_channels = 512, out_channels = 1024, kernel_size = 3, stride = 1)

        self.linear1 = torch.nn.Linear(in_features = 1024*5*8 , out_features = 4096)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(-1, 1024*x.size()[2]*x.size()[3])
        x = self.linear1(x)
        x = self.relu(x)
...
class encoderDecoder(nn.Module):
    def __init__(self, num_lstm_layers, embedding_dim):
        ...
        self.conv_net_stream = ConvNetStream(optical_flow_stream = False, out_features = embedding_dim)
