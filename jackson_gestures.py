...
transcriptions_file_path = '../JIGSAWS/Suturing/transcriptions/'
transcrptions_folder = os.listdir(transcriptions_file_path)
kinematics_file_path = '../JIGSAWS/Suturing/kinematics/AllGestures/'
kinematics_folder = os.listdir(kinematics_file_path)
output_kinematics_file_path = '../JIGSAWS/Suturing/kinematics/AllGesturesWithLabels/'

print(len(transcrptions_folder))
print(len(kinematics_folder))
...
for i in range(len(kinematics_folder)):
    ...
    for ii, line in enumerate(kinematics):
        ...
        nice_line = line.strip().replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ')
        output.write(f'{nice_line} {current_gesture}\n')
    output.close()
    kinematics.close()
